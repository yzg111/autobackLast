 ...
 */
public class ACMQQueueReceiver {
    //日志信息
    private Logger logger = LoggerFactory.getLogger(ACMQQueueReceiver.class);
    ...
    //链接其他消息服务器的时候初始化
    public ACMQQueueReceiver(QueueConnectionFactory queueConnFactory, String queueName) {
        this.queueConnFactory = queueConnFactory;
        this.queueName = queueName;
    }
    //链接默认服务器的时候初始化
    public ACMQQueueReceiver(String queueName) {
        this.queueName = queueName;
    }
    ...
    private void connect2MQ() throws Exception {
        ...
        // 6.create a topic receiver from topic session
        try {
