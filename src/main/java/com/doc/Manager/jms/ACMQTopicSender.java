 ...
 */
public class ACMQTopicSender {
    //日志信息
    private Logger logger = LoggerFactory.getLogger(ACMQTopicSender.class);
    ...
    private Destination queue;//消息的目的地
    ...
    //初始化链接外部服务器的消息发送队列
    public ACMQTopicSender(ConnectionFactory ConnFactory, String queueName) {
        this.queueConnFactory=ConnFactory;
        this.queueName = queueName;
    }

    //初始化链接默认的消息服务器队列
    public ACMQTopicSender(String queueName) {
        this.queueName = queueName;
    }
    ...
    private void connect2MQ() throws Exception {
        ...
        queue = queueSession.createTopic(queueName);//创建消息队列
